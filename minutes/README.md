...
- 音声そのものも端末内に保存できる(既定はオフ。下記「マイクは同時に使えない」を参照)
...
録音がマイクを掴んだままだと、認識側は無言のまま何も返さず「音声はきれいに録れているのに本文が空」になります。
そのためこのアプリは文字起こしを優先します。

- 「音声も保存する」の既定は**オフ**
- オンにした場合も、まず文字起こしだけを始め、**認識が動いているのを確認してから**録音を足す
- 録音を足したあとに認識が止まったら、録音をやめて文字起こしに戻し、
  その端末は「同時利用できない」と記憶して以後は自動でオフにする
...
録音中に問題が起きた場合も、録音画面に原因と対処が表示され、
「認識の状態を見る」から同じ診断情報をコピーできます。
